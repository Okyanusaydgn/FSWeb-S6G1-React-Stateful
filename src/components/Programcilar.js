...
export default function Programcilar() {
  ...
  const [aktifProgramcilar, setAktifProgramcilar] = useState(null);
  ...
  const oneCikaninIsmi = () => {
   return programcilar.find(item =>item.id == aktifProgramcilar).isim;
    // Bunu sona bırakın!
  ...
  const stil = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: aktifProgramcilar == null ?'royalblue' : "gold" // 🤔 kutlarken renk gold'a dönecek
  };
      ...
      <div className='programmers'>
        {
          ...
          programcilar.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.isim} <button onClick={() => {setAktifProgramcilar(dev.id)/* burada dev.id 'yi öne çıkan id'ye atayın */ }}>Kutla</button>
            </div>
          )
        }
      </div>
